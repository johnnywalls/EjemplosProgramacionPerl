...
my $listado_pagos = $schema->resultset('Payment')->search({
  amount => { '>=' => 5 },
  payment_date => { -between => [ '2007-02-12 00:00:00', '2007-02-18 23:59:59' ] }
}, {
...
foreach ( $listado_pagos->all ) {
  printf "%-30s %-30s %-10s %5.2f\n",
         ...
         $_->staff->first_name . ' ' . $_->staff->last_name, 
...
my $listado_peliculas_inventario = $schema->resultset('Film')->search({}, {
